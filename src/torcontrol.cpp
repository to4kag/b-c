    ...
    reconnect_timeout(RECONNECT_TIMEOUT_START)
{
    ...
    if (!conn.Connect(_target, [&](TorControlConnection& conn) { connected_cb(conn); },
            [&](TorControlConnection& conn) { disconnected_cb(conn); })) {
...
void TorController::auth_cb(TorControlConnection& _conn, const TorControlReply& reply)
{
    if (reply.code == 250) {
        ...
        _conn.Command(strprintf("ADD_ONION %s Port=%i,127.0.0.1:%i", private_key, GetListenPort(), GetListenPort()),
            [&](TorControlConnection& _conn, const TorControlReply& reply) { add_onion_cb(_conn, reply); });
    } else {
...
void TorController::authchallenge_cb(TorControlConnection& _conn, const TorControlReply& reply)
{
    if (reply.code == 250) {
        ...
        if (l.first == "AUTHCHALLENGE") {
            ...
            _conn.Command("AUTHENTICATE " + HexStr(computedClientHash), [&](TorControlConnection& _conn, const TorControlReply& reply) { auth_cb(_conn, reply); });
        } else {
    ...
    if (reply.code == 250) {
        ...
        if (!torpassword.empty()) {
            if (methods.count("HASHEDPASSWORD")) {
                ...
                _conn.Command("AUTHENTICATE \"" + torpassword + "\"", [&](TorControlConnection& _conn, const TorControlReply& reply) { auth_cb(_conn, reply); });
            } else {
                LogPrintf("tor: Password provided with -torpassword, but HASHEDPASSWORD authentication is not available\n");
            }
        } else if (methods.count("NULL")) {
            LogPrint(BCLog::TOR, "tor: Using NULL authentication\n");
            _conn.Command("AUTHENTICATE", [&](TorControlConnection& _conn, const TorControlReply& reply) { auth_cb(_conn, reply); });
            ...
            if (status_cookie.first && status_cookie.second.size() == TOR_COOKIE_SIZE) {
                ...
                _conn.Command("AUTHCHALLENGE SAFECOOKIE " + HexStr(clientNonce), [&](TorControlConnection& _conn, const TorControlReply& reply) { authchallenge_cb(_conn, reply); });
            } else {
...
void TorController::connected_cb(TorControlConnection& _conn)
{
    ...
    if (!_conn.Command("PROTOCOLINFO 1", [&](TorControlConnection& _conn, const TorControlReply& reply) { protocolinfo_cb(_conn, reply); }))
...
void TorController::Reconnect()
{
     ...
     * may be restarting.
     */
    if (!conn.Connect(target, [&](TorControlConnection& _conn) { connected_cb(_conn); },
            [&](TorControlConnection& _conn) { disconnected_cb(_conn); })) {
...
void StartTorControl()
{
    ...
    torControlThread = std::thread([&] { TraceThread<void (*)()>("torcontrol", &TorControlThread); });
}
