...
{
    uint64_t hash1 = (CHashWriter(Ser::GETHASH, 0) << nKey << GetKey()).GetCheapHash();
    uint64_t hash2 = (CHashWriter(Ser::GETHASH, 0) << nKey << GetGroup() << (hash1 % ADDRMAN_TRIED_BUCKETS_PER_GROUP)).GetCheapHash();
...
int CAddrInfo::GetNewBucket(const uint256& nKey, const CNetAddr& src) const
{
    ...
    uint64_t hash1 = (CHashWriter(Ser::GETHASH, 0) << nKey << GetGroup() << vchSourceGroupKey).GetCheapHash();
    uint64_t hash2 = (CHashWriter(Ser::GETHASH, 0) << nKey << vchSourceGroupKey << (hash1 % ADDRMAN_NEW_BUCKETS_PER_SOURCE_GROUP)).GetCheapHash();
    return hash2 % ADDRMAN_NEW_BUCKET_COUNT;
}

int CAddrInfo::GetBucketPosition(const uint256 &nKey, bool fNew, int nBucket) const
{
    uint64_t hash1 = (CHashWriter(Ser::GETHASH, 0) << nKey << (fNew ? 'N' : 'K') << nBucket << GetKey()).GetCheapHash();
