...
BOOST_AUTO_TEST_CASE(tx_valid)
{
    ...
    for (unsigned int idx = 0; idx < tests.size(); idx++) {
	    ...
	    for (unsigned int inpIdx = 0; inpIdx < inputs.size(); inpIdx++) {
            ...
            CDataStream stream(ParseHex(transaction), Ser::NETWORK, PROTOCOL_VERSION);
...
BOOST_AUTO_TEST_CASE(basic_transaction_tests)
{
    ...
    CDataStream stream(vch, Ser::DISK, CLIENT_VERSION);
...
static void CreateCreditAndSpend(const CKeyStore& keystore, const CScript& outscript, CTransactionRef& output, CMutableTransaction& input, bool success = true)
{
    ...
    CDataStream ssout(Ser::NETWORK, PROTOCOL_VERSION);
    ssout << outputm;
    ssout >> output;
    ...
    assert(ret == success);
    CDataStream ssin(Ser::NETWORK, PROTOCOL_VERSION);
    ssin << inputm;
    ssin >> input;
